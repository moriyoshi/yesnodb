    ...
    pub(crate) fn call_flight(
        ...
        match verb {
            ...
            "flight_view_select" => {
                ...
                let expression = yesno_flight::SetExpr::ViewSelect { key, view, set };
                let rt = self.rt_handle(verb)?;
                let ep = self.endpoint(h, verb)?;
            ...
            "flight_view_fold" => {
                ...
                let key = a.u64(1)?;
                let view = view_spec(*self.view(a.handle(2)?, verb)?);
                let reduce = match a.str_at(3)? {
                    "any" => yesno_flight::ViewReduce::Any,
                    "all" => yesno_flight::ViewReduce::All,
                    "parity" => yesno_flight::ViewReduce::Parity,
                    other => {
                        return Err(value_err(format!(
                ...
                let expression = yesno_flight::SetExpr::ViewFold { key, view, reduce };
                let rt = self.rt_handle(verb)?;
                let ep = self.endpoint(h, verb)?;
                let (total, rows) = rt.block_on(query_descriptor(
                    ep,
