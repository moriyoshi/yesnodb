...
impl Allocator {
    ...
    /// Retire slab `id` from every class's bump pointer.
    ///
    /// Load-bearing, not tidiness. A slab that reaches [`SlabState::Free`] is a
    /// candidate for `new_slab_for`, which re-initializes it with **a different
    /// class's geometry**. If the class it was previously the bump slab for
    /// still names it in `active`, that class goes on allocating into it with
    /// its own slot size: two slot sizes then index one occupancy bitmap, so
    /// `first_free` hands class A a slot that overlaps a slot class B already
    /// wrote, and `slot_offset` turns the same slot index into two different
    /// cells. One chunk's payload lands where another's `ExtTrailer` should be,
    /// and because `owning_class` reads the slab's single recorded class, the
    /// reader resolves the trailer of the *other* class's slot and reports
    /// "points at another chunk" -- pointing at a healthy extent, having
    /// computed the wrong place to look.
    ///
    /// Observed on 2026-09-15 as slab 20 serving classes 9 and 10 at once
    /// ( `slab-free-leaves-a-stale-bump-pointer` in JOURNAL.md ). The damage
    /// only lands on reuse, which is why it needs churn -- a workload that
    /// empties a slab and then allocates a *different* class -- to show up at
    /// all, and why it survived every test that grew a database monotonically.
    fn retire_from_active(&mut self, id: u32) {
...
mod tests {
    ...
    /// A slab emptied and then reused by a *different* class must not still be
    /// the bump slab of its old one.
    ///
    /// Without `retire_from_active`, `active[old]` survives the slab reaching
    /// `SlabState::Free`, `new_slab_for` re-initializes that very slab with the
    /// new class's geometry, and the old class resumes bump-allocating into it.
    /// Two slot sizes then index one occupancy bitmap and the cells overlap.
    ///
    /// The assertion is deliberately `owning_class` rather than an offset
    /// comparison: `owning_class` is what the read path consults to find an
    /// extent's trailer, so a cell it resolves to the wrong class is precisely
    /// the "points at another chunk" failure, stated at the layer that causes it.
    #[test]
    fn a_reused_slab_is_no_longer_the_bump_slab_of_its_old_class() {
        let (old, new) = (9u8, 10u8);
        assert_ne!(
            class_size(old),
            class_size(new),
            "test needs two geometries"
        );

        let mut a = Allocator::new();

        // Fill a slab for `old` -- not to capacity, so `active[old]` still names
        // it. This is the state the bug needs; a slab retired by being *full*
        // clears `active` on its own.
        let cells: Vec<u64> = (0..4).map(|_| a.alloc(old).unwrap()).collect();
        let slab = slab_of(cells[0]);
        assert!(
            cells.iter().all(|&c| slab_of(c) == slab),
            "the four should share one bump slab"
        );

        // Empty it. The last free takes the slab to `Free`.
        for &c in &cells {
            a.free_now(c, old);
        }
        assert!(matches!(a.slabs[slab as usize].state, SlabState::Free));

        // A different class now claims the freed slab and rewrites its geometry.
        let taken = a.alloc(new).unwrap();
        assert_eq!(
            slab_of(taken),
            slab,
            "the freed slab should be the one reused"
        );

        // The old class must land somewhere else entirely.
        let after = a.alloc(old).unwrap();
        assert_ne!(
            slab_of(after),
            slab,
            "class {old} allocated back into a slab now owned by class {new}"
        );
        assert_eq!(a.owning_class(after), Some(old));
    ...
    /// events: `free_now` runs during reclamation, `adopt_live_at_open` runs once
    /// when a shard is reopened and the liveness map says a slab holds nothing.
    /// Deleting the second one leaves **the entire suite green** ( measured
    /// 2026-09-15: 1069 tests, 0 failures ), because every other test reaches the
    /// allocator through the free path. A fix with two call sites needs two
    /// sabotage-checked tests; one test covering one site is a fix that is half
    /// unguarded, and it looks exactly like a fix that is whole.
    #[test]
    fn a_slab_emptied_at_open_is_no_longer_the_bump_slab_of_its_old_class() {
        let (old, new) = (9u8, 10u8);
        let mut a = Allocator::new();
        ...
        // Reopen with a liveness map that retains nothing in this slab. That is
        // the honest shape: `adopt_live_at_open` takes what a rebuild found, and
        // a rebuild that found no reachable entry here reports an empty set.
        let live: BTreeMap<u32, BTreeSet<u32>> = BTreeMap::new();
        a.adopt_live_at_open(&live);
        assert!(
    ...
    /// audit. It matters more than its size suggests: `alloc` derives a slot
    /// offset from the class, so a class the ladder does not define would
    /// compute an address from `None` if this guard were removed or reordered.
    #[test]
