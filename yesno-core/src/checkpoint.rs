...
//! 4. Bottom-up copy-on-write rebuild of the index, reusing every leaf no key
//!    touched **without decoding it** ( `Tree::build_updating` ).
...
pub struct CheckpointResult {
    ...
    /// Old refs of entries the new index does not carry.
    ///
    /// Gathered during the index merge, where every previous entry is visited
    /// anyway. The caller queues these for reclamation **after** `run` returns;
    /// that is safe because `defer_free` and `supersede_packed_chunk` only
    /// enqueue, so nothing freed here can be handed back out by the allocations
    /// this same checkpoint already made.
...
pub fn run(
    ...
    let mut entries: Vec<(ChunkKey, ChunkRef)> = Vec::with_capacity(dirty.len());
    ...
    // happens there rather than the caller building a second copy of the whole
    // key space. See `Tree::build_updating`.
    ...
    let tree = upd.tree;
    res.carried = upd.carried;
    res.superseded = upd.superseded;
