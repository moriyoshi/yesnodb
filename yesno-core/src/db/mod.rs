...
use crate::events::{self, CoreEvent, DatabaseMode, EventError, Events};
...
impl Shard {
    ...
    /// The store's superblock sequence, which every checkpoint advances.
    ///
    /// Used to invalidate a prefetch: see `WriteBatch::commit`. Cheap, but it
    /// takes the store lock, so read it **once** per shard rather than per row.
    fn store_seq(&self) -> Option<u64> {
        Some(self.store.as_ref()?.lock().unwrap().superblock().seq)
    }
...
struct DbInner {
    ...
    /// Executor for a commit's per-shard work. Sequential unless the embedder
    /// supplied one; this crate never creates threads.
    dispatch: Dispatcher,
...
pub struct DbOptions {
    ...
    /// Where operational events go. Defaults to discarding them.
    ///
    /// A field rather than a constructor argument: `open_with_events` was one
    /// `open_with_*` per collaborator, which does not compose -- two
    /// collaborators would have needed a third constructor naming both.
    pub events: Events,

    /// Executor for a commit's per-shard work. Defaults to sequential.
    ///
    /// `yesno-core` spawns no threads of its own; supplying an executor is how
    /// a host that already has one lends it. See [`crate::dispatch`] for the
    /// contract and for the measurements that say when it is worth doing.
...
impl Db {
    ...
    pub fn open_with(dir: impl AsRef<std::path::Path>, opts: DbOptions) -> Result<Self> {
        Self::open_observed(dir.as_ref(), opts.clone(), OpenMode::Writer)
    }
    ...
    pub fn open_with_events(
        ...
        // Kept as a thin wrapper rather than removed: `opts.events` is the one
        // place a sink lives now, and this sets it. A collaborator added later
        // needs no constructor of its own, which is what this shape could not do.
        opts.events = sink;
        Self::open_observed(dir.as_ref(), opts.clone(), OpenMode::Writer)
    }
    ...
    pub fn open_replica(dir: impl AsRef<std::path::Path>, opts: DbOptions) -> Result<Self> {
        Self::open_observed(dir.as_ref(), opts.clone(), OpenMode::Replica)
    }
    ...
    pub fn open_reader_with(dir: impl AsRef<std::path::Path>, opts: DbOptions) -> Result<Self> {
        Self::open_observed(dir.as_ref(), opts.clone(), OpenMode::Reader)
    }

    fn open_observed(dir: &std::path::Path, opts: DbOptions, mode: OpenMode) -> Result<Self> {
        let sink = opts.events.clone();
        ...
        match Self::open_inner(dir, opts.clone(), mode, sink.clone()) {
            Ok(db) => {
                events::emit(
                    &sink,
    ...
    fn checkpoint_inner(&self, operation_id: events::OperationId) -> Result<Version> {
        ...
        for (shard_id, shard) in self.inner.shards.iter().enumerate() {
            ...
            // join them below, because evacuation rewrites an untouched chunk
            // into the current generation and its old entry must not survive.
            //
            // Moved by value rather than cloned: nothing below reads `touched`
            // again, and a clone of it is an allocation proportional to the
            // dirty set inside the region this change exists to shorten.
            let mut superseded_keys: std::collections::BTreeSet<ChunkKey> = touched;
            if let Some(t) = store.tree() {
                // The superseded refs are **not** gathered here.
                //
                // The obvious fused version looked them up per touched key with
                // `Tree::get`, which is `O( dirty log n )` and reads 2-3 nodes
                // per descent. Measured 2026-09-15 at ~0.7 us each: **62%** of
                // the exclusive hold at 10 000 touched keys, making the fused
                // path *slower* than the whole-key-space scan it replaced past
                // roughly 1 300 dirty keys. `Tree::build_updating` now returns
                // them instead, gathered in a merge that visits every previous
                // entry anyway -- a sequential pass already happening beats a
                // random descent per key.

                // Evacuation is the one thing that still needs to see untouched
                // entries, and it is off by default ( `EVACUATE_PER_CHECKPOINT`
                // is 0 ), so the scan is skipped entirely unless asked for.
                if !evacuating.is_empty() {
                    ...
                    for (ck, cref) in carried {
                        if superseded_keys.contains(&ck) {
                            continue;
                        }
            ...
            // Queue at `w`, not at "now": an older snapshot may still reach this
            // extent through the previous root, so it becomes free only once no
            // reader can be below the watermark *and* the A/B superblock has
            // cycled twice.
            //
            // **After `run`, not before.** The refs come from the index merge
            // inside it. Safe in this order because both calls below only
            // *enqueue*: nothing released here can be handed back out by the
            // allocations `run` has already made.
            for cref in &res.superseded {
                let Some(cell) = cref.cell() else { continue };
                // A packed chunk owns no slot; its page is the unit, and it is
                // returned only once every chunk in it is dead. Without this
                // branch packed pages were allocated, never accounted and never
                // freed — which is what the aged-state measurement finally
                // caught.
                if store.allocator().is_packed_cell(cell) {
                    // See the twin of this in `checkpoint.rs`: a run's length is
                    // a dependent read of its `nruns` prefix, and
                    // `payload_len(None).unwrap_or(0)` silently returned zero
...
impl WriteBatch {
    ...
    ///
    /// **Between those two endpoints the fold has a closed form.** For a
    /// *randomly* scattered set of density `d` over a contiguous range, the
    /// expected run count is `N·d·( 1 - d )` against `N·d` members, so
    ///
    /// ```text
    ///   ordinals per run  =  1 / ( 1 - d )
    /// ```
    ///
    /// which is 1.05x at `d = 0.05`, 2x at `d = 0.5`, and 10x at `d = 0.9`.
    /// Reported by a consumer measuring their own ordinal sets at `d = 0.5`
    /// and sparse ( 2.00x and 1.02x, invariant to batch size and width,
    /// 2026-09-18 ) and re-derived here against a simulation over two orders
    /// of magnitude of `N`. **So this API repays density, and a caller whose
    /// sets are sparse — which is usually the caller who most wants ingest
    /// speed — should expect roughly no fold at all** and decide on that basis
    /// rather than on the existence of a range operation.
    ///
    /// **Density is not the same lever as contiguity, and the difference is
    /// much larger than the formula.** The fold above is bounded by
    /// `1 / ( 1 - d )` because the gaps are random; ordinals that are
    /// *contiguous by construction* collapse to a single operation whatever
    /// their width. The same consumer measured a whole-row clear through
    /// [`WriteBatch::remove_range`] — `row_bits` contiguous ordinals — falling
    /// from `row_bits` operations to one, about 2x on their delete path
    /// end to end. An operation count cannot distinguish the two cases, which
    /// is what makes it the wrong thing to reason from: ask whether the
    /// ordinals are adjacent by design, not whether there are many of them.
    ...
    pub fn commit(self) -> Result<Committed> {
        ...
        // Resolve, **outside every memtable lock**, the chunks this batch will
        // ask the store for.
        //
        // `Memtable::insert_range` and friends take a `base` closure and call it
        // for each prefix the memtable lacks. That closure was `Shard::disk_chunk`,
        // which takes the store mutex, descends the B+tree and reads a container
        // with checksum verification -- all inside `mem.write()`, once per row.
        // Measured 2026-09-16: **71-80% of the exclusive hold**, and it fires even
        // for keys that do not exist, because a descent is how absence is learned.
        //
        // Safe to read early under I2: a published extent is immutable, so a value
        // read now cannot change underneath, only become *superseded* -- and the
        // memtable is consulted first and is authoritative for anything newer.
        // The one thing that can invalidate it is a concurrent **checkpoint**
        // persisting and evicting a chunk between here and the apply, which the
        // superblock sequence below detects; a concurrent *commit* cannot, since
        // this holds every participant's `write` guard.
        //
        // Capped because this holds every prefetched container at once where the
        // apply loop reads and drops them one at a time. Beyond the cap the
        // closure falls back to reading under the lock, which is what it did
        // before, so a very wide range degrades to the old behaviour rather than
        // to a memory spike.
        const PREFETCH_MAX: usize = 4096;
        type ShardPrefetch = (
            u64,
            std::collections::HashMap<ChunkKey, Option<Container>>,
            std::collections::HashMap<u64, Vec<u64>>,
        );
        let mut prefetch: std::collections::HashMap<usize, ShardPrefetch> =
            std::collections::HashMap::new();
        // Resolved concurrently when the embedder supplied an executor. The
        // tasks touch disjoint state -- task `i` reads shard `participants[i]`,
        // and each shard owns its own store lock -- so no ordering is needed.
        //
        // Each writes its own slot rather than a shared map, which is what keeps
        // the closure `Fn` and needs no lock between tasks.
        let slots: Vec<std::sync::Mutex<Option<ShardPrefetch>>> = (0..participants.len())
            .map(|_| std::sync::Mutex::new(None))
            .collect();
        let ran = std::sync::atomic::AtomicUsize::new(0);
        self.handle.inner.dispatch.run(participants.len(), &|task| {
            ran.fetch_add(1, Ordering::Relaxed);
            let s = participants[task];
            let shard = &self.handle.inner.shards[s];
            let Some(store) = shard.store.as_ref() else {
                return;
            };
            let mut want: Vec<ChunkKey> = Vec::new();
            let mut del_keys: Vec<u64> = Vec::new();
            'units: for unit in &planned[&s] {
                match unit {
                    Planned::Range { key, lo, hi, .. } => {
                        let (a, b) = (lo >> crate::CHUNK_BITS, hi >> crate::CHUNK_BITS);
                        for pfx in a..=b {
                            if want.len() >= PREFETCH_MAX {
                                break 'units;
                            }
                            want.push(ChunkKey::new(*key, pfx));
                        }
                    }
                    Planned::Values { key, prefix, .. } => {
                        if want.len() >= PREFETCH_MAX {
                            break 'units;
                        }
                        want.push(ChunkKey::new(*key, *prefix));
                    }
                    // A whole-key delete needs the prefixes already on disk so
                    // each gets a tombstone; without them the persisted value
                    // shows through the delete. `key_prefixes` is a **range scan
            ...
            if let Some(tree) = st.tree() {
                for ck in want {
                    ...
                    let v = tree
                        ...
                        .ok()
                        .flatten()
                        .and_then(|cref| st.read_container_for(ck, cref).ok().flatten());
                    map.insert(ck, v);
                }
            }
            let mut dels = std::collections::HashMap::with_capacity(del_keys.len());
            for k in del_keys {
                dels.entry(k)
                    .or_insert_with(|| st.key_prefixes(k).unwrap_or_default());
            }
            drop(st);
            *slots[task].lock().unwrap() = Some((seq, map, dels));
        });
        // **The executor is not trusted to have run everything.** Skipping a
        // task is memory-safe and silently drops a shard's resolved chunks:
        // the apply would then read them under the lock instead -- slower but
        // correct -- or, for a delete, build the wrong tombstone set, which is
        // not. Refuse rather than find out later. See `crate::dispatch`.
        if ran.load(Ordering::Relaxed) != participants.len() {
            return Err(CodecError::Invariant(
                "the supplied Dispatch did not run every task",
            ));
        }
        for (task, slot) in slots.into_iter().enumerate() {
            if let Some(v) = slot.into_inner().unwrap() {
                prefetch.insert(participants[task], v);
            }
        }

        // Applied concurrently when the embedder supplied an executor.
        //
        // Task `i` owns shard `participants[i]` entirely: it takes that shard's
        // memtable write lock and touches nothing another task does, so no
        // ordering between tasks is required. Version assignment already
        // happened, once, above -- I5 is a property of *that*, not of the order
        // these run in.
        //
        // This is the reader-facing half. The per-shard holds are taken one at a
        // time today, so a reader scanning every shard waits their **sum**;
        // running them concurrently makes it wait the **max**. Measured
        // 2026-09-17, the sum is invariant to shard count at about 100 us while
        // the per-shard hold falls from 97.89 us at one shard to 3.11 us at
        // thirty-two, so the reduction available is 7x at eight shards and 32x
        // at thirty-two -- and it is only worth taking with an executor cheap
        // enough, which is why this crate supplies none. See `crate::dispatch`.
        let changed_slots: Vec<AtomicU64> =
            (0..participants.len()).map(|_| AtomicU64::new(0)).collect();
        let applied = std::sync::atomic::AtomicUsize::new(0);
        self.handle.inner.dispatch.run(participants.len(), &|task| {
            applied.fetch_add(1, Ordering::Relaxed);
            let s = participants[task];
            let mut local = 0u64;
            // One store-lock acquisition per shard, not per row: if a checkpoint
            // has advanced the superblock since the prefetch, every entry is
            // discarded and the closure reads under the lock as before.
            let fresh = match prefetch.get(&s) {
                Some((seq, _, _)) => self.handle.inner.shards[s].store_seq() == Some(*seq),
                None => false,
            };
            let pre = if fresh {
                prefetch.get(&s).map(|(_, m, _)| m)
            } else {
                None
            };
            let pre_del = if fresh {
                prefetch.get(&s).map(|(_, _, d)| d)
            } else {
                None
            };
            ...
            for unit in &planned[&s] {
                ...
                let op = match unit {
                    Planned::Range {
                        ...
                        local += if *remove {
                            mem.remove_range(*key, *lo, *hi, version, |p| {
                                match pre.and_then(|m| m.get(&ChunkKey::new(*key, p))) {
                                    Some(v) => v.clone(),
                                    None => shard.disk_chunk(*key, p),
                                }
                            })
                        } else {
                            mem.insert_range(*key, *lo, *hi, version, |p| {
                                match pre.and_then(|m| m.get(&ChunkKey::new(*key, p))) {
                                    Some(v) => v.clone(),
                                    None => shard.disk_chunk(*key, p),
                                }
                    ...
                    Planned::Values {
                        ...
                        local += mem.apply_values(*key, *prefix, vals, *remove, version, || {
                            match pre.and_then(|m| m.get(&ChunkKey::new(*key, *prefix))) {
                                Some(v) => v.clone(),
                                None => shard.disk_chunk(*key, *prefix),
                            }
                        });
                        continue;
                    }
                ...
                match op {
                    ...
                    Op::DeleteKey(k) => {
                        ...
                        let on_disk = match pre_del.and_then(|d| d.get(k)) {
                            Some(v) => v.clone(),
                            None => self.handle.inner.shards[s]
                                .store
                                .as_ref()
                                .map(|st| {
            ...
            changed_slots[task].store(local, Ordering::Relaxed);
        });
        // The same refusal as the prefetch, and it matters more here: a skipped
        // task means a shard's rows were never applied, while its WAL records
        // below say they were. That is a divergence recovery would not detect.
        if applied.load(Ordering::Relaxed) != participants.len() {
            return Err(CodecError::Invariant(
                "the supplied Dispatch did not run every task",
            ));
        }
        let changed: u64 = changed_slots
            .iter()
...
mod tests {
    ...
    ///
    /// `packed-page-sharing-may-contend-across-keys` predicted that one write
    /// invalidates verification for every key sharing the page, with the cost
    /// scaling as keys-per-page. The mechanism is real and
    /// `a_write_into_a_packed_page_invalidates_it_for_every_key_in_it` pins it
    /// -- but that test reaches it by calling `write_at` on a live page
    /// directly, which **I2 means the engine never does**. A checkpoint builds
    /// pages fresh at newly allocated cells and publishes them immutably, so a
    /// live page is never written a second time and there is nothing to evict.
    ///
    /// Measured 2026-09-15 across keys-per-page of 64, 21.3, 9.1 and 2.0
    /// ( payloads 8 B to 1800 B ): evictions were **0 in every configuration**,
    /// and re-verifications after the write were a constant 3 -- the new page
    /// plus two rewritten index nodes -- rather than scaling with sharing.
    /// Under sustained churn, where cells are recycled and a write *can* land
    /// on a verified region, eviction runs at **one region per checkpoint**
    /// independent of keys-per-page, and only on pages already dead: 400/400
    /// reads succeeded every round and `verify()` stayed clean.
    ///
    /// **Not sabotage-verified in the usual sense, and that is stated rather
    /// than glossed**: there is no line whose removal reddens it, because it
    /// asserts the *absence* of an interaction. Its guard is the pairing -- the
    /// companion test proves the eviction machinery works, this proves the
    /// write path does not reach it. Read them together or neither means much.
    #[test]
    fn a_checkpoint_evicts_no_verified_region_however_many_keys_share_a_page() {
        ...
        // and make the keys-per-page axis inexpressible.
        let mk = |k: u64| -> Vec<u64> { (0..200u64).map(|i| (k << 20) | (i * 7)).collect() };

        {
            ...
            .unwrap();
            for k in 0..KEYS {
                db.insert_many(k, &mk(k)).unwrap();
            }
            db.checkpoint().unwrap();
        }

        // Reopen so the memtable serves nothing and every read reaches storage.
        let db = Db::open_with(
            &dir,
            DbOptions {
                shards: 1,
        ...
        let read_all = || {
            ...
            for k in 0..KEYS {
                let _ = s.load(k).unwrap();
            }
        };

        read_all();
        let before = verified();
        // CONTROL: the row is about packed pages. At 400 B per chunk these
        // occupy several pages, so a count at or below the index-node count
        // would mean the fixture never exercised chunk storage at all.
        assert!(
            before > 4,
            "fixture verified only {before} regions -- too few to involve packed pages"
        );

        db.insert_many(0, &[7_777_777]).unwrap();
        db.checkpoint().unwrap();

        assert!(
            verified() >= before,
            "a checkpoint evicted a verified region ({before} -> {}); a published \
             page was written a second time, which I2 forbids",
            verified()
        );
    }
    ...
    /// Every checkpoint advances the superblock sequence.
    ///
    /// # What depends on this, and why it is asserted rather than assumed
    ///
    /// `WriteBatch::commit` prefetches the chunks a batch will need **outside**
    /// every memtable lock, then discards the prefetch if this shard's store has
    /// moved since. The hazard it guards is narrow but real: a concurrent
    /// checkpoint can persist a memtable chunk and evict it between the prefetch
    /// and the apply, after which the memtable no longer has the chunk and the
    /// prefetched copy is older than what the store now holds.
    ///
    /// The guard is `store_seq()` comparison, so **the guard is only as good as
    /// the sequence advancing**. If a checkpoint ever stopped bumping it, the
    /// comparison would silently always succeed and the stale prefetch would be
    /// used -- a wrong answer with nothing to catch it.
    ///
    /// The guard's own firing is not asserted here and that is stated rather
    /// than glossed: reaching it needs a checkpoint to land inside a window of a
    /// few microseconds, and `checkpoints_concurrent_with_writers_lose_nothing`
    /// was measured hitting it **zero** times. Testing the firing would need a
    /// fault-injection seam in the commit path, which is not worth a seam in
    /// production code; testing the assumption it rests on is cheap and is what
    /// would actually break silently.
    /// A concurrent executor must produce exactly what the sequential one does.
    ///
    /// The whole escape hatch rests on the per-shard work being independent.
    /// This is the assertion that says so: same corpus, same batches, one
    /// database resolving its chunks sequentially and one across scoped
    /// threads, compared key by key.
    #[test]
    fn a_concurrent_dispatcher_commits_what_the_sequential_one_does() {
        use crate::dispatch::{Dispatch, Dispatcher};
        ...
        let build = |name: &str, d: Dispatcher| -> (std::path::PathBuf, Db) {
            ...
            for round in 0..3u64 {
                let mut b = db.batch();
                for k in 0..KEYS {
                    b.insert(k, (k << 20) | round);
                    b.insert(k, (k << 20) | (round + 100));
                }
                b.commit().unwrap();
                db.checkpoint().unwrap();
            }
            // Deletes too: their tombstone set comes from the same prefetch.
            let mut b = db.batch();
        ...
        for k in 0..KEYS {
            ...
            let (x, y) = (sa.load(k).unwrap(), sb.load(k).unwrap());
            assert_eq!(
                x.iter().collect::<Vec<_>>(),
                y.iter().collect::<Vec<_>>(),
                "key {k} differs between sequential and concurrent dispatch"
            );
        }
        // A control: the corpus must be non-empty, or the comparison above is
        // two empty databases agreeing.
        assert!(
            !a.snapshot().unwrap().load(1).unwrap().is_empty(),
            "the fixture wrote nothing, so the comparison proves nothing"
        );
    }

    /// An executor that skips tasks is refused, not silently obeyed.
    ///
    /// Skipping is memory-safe and is the one contract violation the borrow
    /// checker cannot prevent -- returning early while tasks still run is not
    /// expressible, because the closure is borrowed rather than `'static`. A
    /// skipped task drops a shard's resolved chunks, which for a delete means
    /// the wrong tombstone set and a persisted value showing through. So the
    /// commit counts completions rather than trusting the implementation.
    #[test]
    fn a_dispatcher_that_skips_work_is_refused() {
        use crate::dispatch::{Dispatch, Dispatcher};

        struct Lazy;
        impl Dispatch for Lazy {
            fn run(&self, n: usize, f: &(dyn Fn(usize) + Sync)) {
                // Runs all but the last: the shape a buggy work-stealing pool
                // would have, and the one that is silent without this check.
        ...
        let err = b.commit().expect_err("a skipped task must fail the commit");
        assert!(
            matches!(
                err,
                CodecError::Invariant("the supplied Dispatch did not run every task")
            ),
    ...
    fn a_checkpoint_advances_the_store_sequence() {
        ...
        .unwrap();
        let seq = || db.inner.shards[0].store_seq().unwrap();
        ...
        assert!(
            after > before,
            "a checkpoint must advance the store sequence ({before} -> {after}); \
             the commit path's prefetch guard is a comparison against it"
        );

        // And again, so a one-shot bump cannot pass.
        db.insert_many(2, &[40]).unwrap();
        db.checkpoint().unwrap();
        assert!(seq() > after, "the second checkpoint must advance it too");
    }
