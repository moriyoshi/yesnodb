...
//! ## What is *not* here
//!
//! **Two vector arms: AArch64 NEON and x86_64 SSE4.2.** Every other target runs
//! the scalar merge.
//!
//! # The x86 arm, measured on real silicon 2026-09-18
//!
//! Intel Core i9-9880H ( Coffee Lake ), `x86_64-apple-darwin`, via
//! `intersect_crossover`. `array/generic` against the same build with the x86
//! dispatch forced off, so the pair is one binary shape apart:
//!
//! ```text
...
//! ```
//!
//! # `pcmpestrm`, and why the first version of this arm was wrong to avoid it
//!
//! This module shipped a rotate ladder on 2026-09-17 -- eight compares against
//! eight rotations of `vb` -- with `_mm_cmpestrm` declined on the grounds that
//! its case is throughput and no x86 hardware was available to check it. That
//! reasoning was sound and its conclusion did not survive contact with a
//! machine: `pcmpestrm` is **1.09x to 1.37x** faster than the ladder at every
//! size and on both dense and sparse operands, so the ladder is gone.
//!
//! It is also the *simpler* kernel here, which the throughput framing obscured.
//! `pcmpestrm` returns the lane mask **directly**, and all four arms want that
//! mask -- a popcount, a zero test, an accumulator, a `SHUFFLE` index. The NEON
//! arm builds it from eight compares and then narrows it with `lane_bits`;
//! there is nothing to narrow here, so the compaction arm loses a step.
//!
//! **AVX2 was measured and rejected**: sixteen lanes needs a cross-lane permute
//! per rotation, and it came in *below* `pcmpestrm` at every size but one. The
//! wider register buys exactly what the permute costs. Do not re-run this
//! without a different formulation than an all-pairs widening.
//!
//! # What emulation said, and why it is recorded here
//!
//! Before the hardware existed this arm was verified under `qemu-x86_64`, which
//! establishes correctness and **cannot** establish speed. Measured rather than
//! asserted: one binary run natively and under `qemu-aarch64` moved the
//! scalar-to-vector ratio from **1.62x to 0.87x**. Emulation does not add noise
//! to a SIMD comparison, it reverses the verdict -- a translated 8-lane
//! operation becomes a serial host sequence, so width buys nothing while the
//! instruction count still costs. qemu predicted this arm was a regression; it
//! is up to 8.5x. Keep the qemu route for correctness, never for a number.
//!
//! Each arm is selected by its own runtime feature check, so the crate builds
//! and is correct anywhere. On `aarch64-unknown-linux-gnu` `neon` is a **baseline**
...
///
/// Eight `u16` is one 128-bit register — NEON `uint16x8_t` or SSE `__m128i` —
/// which is the whole reason the arm exists at that width and not another. The
/// two architectures agree on the width by coincidence of register size, not by
/// design, so a future 256-bit arm would need its own constant rather than a
/// change here.
#[cfg(any(target_arch = "aarch64", target_arch = "x86_64"))]
const SIMD_BLOCK: usize = 8;

/// For each 8-bit lane mask, the byte indices that compact the matching `u16`
/// lanes of a register to its front.
///
/// Shared by both vector arms, and **not** by coincidence: AArch64's
/// `vqtbl1q_u8` yields zero for an out-of-range index, and x86's `pshufb`
/// yields zero for an index whose high bit is set. `0xFF` satisfies both, so
/// one table drives both compactions and the unmatched tail is deterministic
/// garbage rather than undefined on either — and it is overwritten by the next
/// block's store regardless.
#[cfg(any(target_arch = "aarch64", target_arch = "x86_64"))]
...
/// Nothing here is for outside use: [`SHUFFLE`] is the one shared item and it
/// now lives in the parent module, because both vector arms index it. Sharing
/// it rather than duplicating it is a size decision, not a style one — it is
/// 4 KiB of static, and two copies would also be two things to keep in step
/// with `SIMD_BLOCK`.
#[cfg(target_arch = "aarch64")]
pub(crate) mod simd {
    use super::{SHUFFLE, SIMD_BLOCK};
...
///
/// # Why this mirrors the NEON arm rather than using `pcmpestrm`
///
/// The published SIMD-intersection work, and this crate's own backlog, name
/// SSE4.2's `_mm_cmpestrm` as the natural x86 formulation: it does the 8x8
/// all-pairs compare in one instruction where this does eight. **It is
/// deliberately not used here, and the reason is that its advantage is a
/// throughput claim and no throughput measurement is available on this
/// machine** — see the module header. `pcmpestrm` is microcoded on several
/// microarchitectures and is not uniformly faster than the compare-and-rotate
/// form; choosing it unmeasured would be choosing the harder correctness
/// argument for a speed that is assumed.
///
/// What this form buys instead is that every invariant below is the *same*
/// invariant the NEON arm states, established by the same block rule, so the
/// reasoning transfers rather than being rebuilt. `_mm_cmpestrm` remains an
/// open measurement question, not a rejected one.
///
/// # Width
///
/// `SIMD_BLOCK` is 8 because `__m128i` holds eight `u16`, which is the same
/// arithmetic that makes it 8 on NEON. AVX2 would make it 16 and is likewise a
...
pub(crate) mod simd {
    ...
    /// One bit per lane of `probe`: bit `k` is set iff `probe[k]` occurs
    /// anywhere in `set`.
    ///
    /// `pcmpestrm` in equal-any/bit-mask mode does the whole 8x8 all-pairs
    /// product in one instruction and **returns the lane mask directly**, which
    /// is the form all four arms want -- a popcount for the count, a zero test
    /// for the predicate, an accumulator for containment, and a `SHUFFLE` index
    /// for the compaction. The NEON arm has to build that mask out of eight
    /// compares and then narrow it; here there is nothing to narrow.
    ///
    /// **Argument order is load-bearing**: the mask indexes the *second*
    /// operand. `match_bits( set, probe )` therefore asks "which lanes of
    /// `probe` are in `set`", and swapping the two silently answers the other
    /// question -- which matters only for `contains_all`, the asymmetric arm.
    ///
    /// `pcmpestrm` and not `pcmpistrm`: the implicit-length form treats a zero
    /// element as end-of-string, and `0` is an ordinary value in a chunk, so it
    /// would truncate any block containing one. Faster and wrong.
    ///
    /// # Safety
    ///
    /// Requires `sse4.2`, which the caller establishes.
    #[inline]
    #[target_feature(enable = "sse4.2")]
    unsafe fn match_bits(set: __m128i, probe: __m128i) -> u32 {
        const MODE: i32 = _SIDD_UWORD_OPS | _SIDD_CMP_EQUAL_ANY | _SIDD_BIT_MASK;
        // No `unsafe` block: register-to-register, and `#[target_feature]`
        // establishes the only precondition these intrinsics have.
        let m = _mm_cmpestrm::<MODE>(set, SIMD_BLOCK as i32, probe, SIMD_BLOCK as i32);
        _mm_extract_epi32::<0>(m) as u32
    }

    /// `|a INTERSECT b|` over the merge shape.
    ///
    /// # Safety
    ///
    /// Requires `sse4.2`, which the caller establishes.
        ...
        if na >= SIMD_BLOCK && nb >= SIMD_BLOCK {
            ...
            unsafe {
                while i < ia && j < ib {
                    ...
                    n += match_bits(vb, va).count_ones();
                    // Advance the side whose block ends lower; on a tie both,
                    // since the shared maximum has already been counted. As on
                    // NEON the tie is a throughput choice and not a correctness
                    // one -- sabotaging `bmax <= amax` to `bmax < amax` leaves
                    // every test green, because the un-advanced side simply
                    // spends one extra iteration. Do not read the tests passing
                    // as evidence that this line is right.
                    let amax = *a.get_unchecked(i + SIMD_BLOCK - 1);
                    let bmax = *b.get_unchecked(j + SIMD_BLOCK - 1);
                    i += usize::from(amax <= bmax) * SIMD_BLOCK;
                    j += usize::from(bmax <= amax) * SIMD_BLOCK;
    ...
    /// `a INTERSECT b = the empty set` over the same merge shape, exiting at the
    /// first block that shares a value.
    ///
    /// The exit is per block, not per element, for the reason the NEON arm
    /// states: a short-circuiting reduction cannot be widened, so a coarse exit
    /// beats one that costs the vector arm.
    ///
    /// # Safety
    ///
    /// Requires `sse4.2`, which the caller establishes.
    ...
    pub(super) unsafe fn is_disjoint(a: &[u16], b: &[u16]) -> bool {
        ...
        if na >= SIMD_BLOCK && nb >= SIMD_BLOCK {
            let (ia, ib) = (na - SIMD_BLOCK + 1, nb - SIMD_BLOCK + 1);
            // SAFETY: B1, exactly as in `merge_cardinality`.
            unsafe {
                while i < ia && j < ib {
                    debug_assert!(i + SIMD_BLOCK <= na && j + SIMD_BLOCK <= nb, "B1");
                    let va = _mm_loadu_si128(a.as_ptr().add(i).cast());
                    let vb = _mm_loadu_si128(b.as_ptr().add(j).cast());
                    if match_bits(vb, va) != 0 {
                        return false;
                    }
                    let amax = *a.get_unchecked(i + SIMD_BLOCK - 1);
                    let bmax = *b.get_unchecked(j + SIMD_BLOCK - 1);
    ...
    /// `a INTERSECT b` over the merge shape, appended to `out`.
    ///
    /// The mask `pcmpestrm` returns is already the `SHUFFLE` index, so unlike
    /// the NEON arm there is no narrowing step between the compare and the
    /// compaction store.
    ///
    /// # Safety
    ///
    /// Requires `sse4.2` and `ssse3`, and requires `out` to be empty with
    /// capacity at least `min( a.len(), b.len() ) + SIMD_BLOCK` -- bound **B2**.
        ...
        if na >= SIMD_BLOCK && nb >= SIMD_BLOCK {
            ...
            unsafe {
                while i < ia && j < ib {
                    ...
                    let va = _mm_loadu_si128(a.as_ptr().add(i).cast());
                    let vb = _mm_loadu_si128(b.as_ptr().add(j).cast());
                    // Mask over `va`'s lanes, which is what gets compacted.
                    let bits = match_bits(vb, va) as usize & 0xFF;
                    let shuf = _mm_loadu_si128(SHUFFLE[bits].as_ptr().cast());
                    _mm_storeu_si128(dst.add(k).cast(), _mm_shuffle_epi8(va, shuf));
                    k += bits.count_ones() as usize;
                    let amax = *a.get_unchecked(i + SIMD_BLOCK - 1);
                    let bmax = *b.get_unchecked(j + SIMD_BLOCK - 1);
                    i += usize::from(amax <= bmax) * SIMD_BLOCK;
    ...
    /// Whether every value of `b` occurs in `a`.
    ///
    /// Containment is **asymmetric**, so this is not `is_disjoint` inverted and
    /// its block rule is not that one's: `b` drives, one block of `b` may be
    /// decided by several blocks of `a`, and matches must therefore accumulate
    /// across `a`-advances. The scalar tail is only sound if `i` is rewound to a
    /// point below every undecided `b` value. Both invariants are the NEON
    /// arm's, stated there at length.
    ///
    /// Note the argument order to `match_bits`: here the mask must index `vb`,
    /// so `va` is the set and `vb` the probe -- the opposite of every other arm.
    ///
    /// # Safety
    ///
    /// Requires `sse4.2`, which the caller establishes.
    #[target_feature(enable = "sse4.2")]
    pub(super) unsafe fn contains_all(a: &[u16], b: &[u16]) -> bool {
        const ALL: u32 = (1 << SIMD_BLOCK) - 1;
        let (na, nb) = (a.len(), b.len());
        let (mut i, mut j) = (0usize, 0usize);
        if na >= SIMD_BLOCK && nb >= SIMD_BLOCK {
            let (ia, ib) = (na - SIMD_BLOCK + 1, nb - SIMD_BLOCK + 1);
            // SAFETY: B1. Every advance below re-tests its bound before loading.
            unsafe {
                'blocks: while j < ib {
                    // Skip whole `a` blocks that end below this block's first
                    // value. This is what establishes `a[..i] < b[j]`.
...
fn merge_and(a: &[u16], b: &[u16]) -> Vec<u16> {
    ...
    if std::arch::is_x86_feature_detected!("sse4.2") {
        // `+ SIMD_BLOCK` is bound B2, for the same reason: `pshufb` stores a
        // whole register whatever the match count.
        let mut out = Vec::with_capacity(a.len().min(b.len()) + SIMD_BLOCK);
        // SAFETY: `ssse3` was just detected, and `out` is empty with the
        // capacity B2 requires.
        unsafe { simd::merge_and_into(a, b, &mut out) };
        return out;
    }
...
mod tests {
    ...
    /// Asserts this host can reach the vector arm, so a direct-call wrapper
    /// below cannot silently degrade into testing the scalar path.
    ///
    /// Both arches ship the arm unconditionally in practice -- every AArch64
    /// this crate targets has `neon`, and `ssse3` predates every x86_64 in
    /// service -- so a failure here means the detection changed, not that the
    /// host is exotic.
    #[cfg(any(target_arch = "aarch64", target_arch = "x86_64"))]
    fn assert_vector_arm_reachable() {
        #[cfg(target_arch = "aarch64")]
        assert!(
        ...
        assert!(
            ...
            "every x86_64 this crate targets has ssse3"
        );
    }
    ...
    fn vec_and(a: &[u16], b: &[u16]) -> Vec<u16> {
        assert_vector_arm_reachable();
        // `+ SIMD_BLOCK` is bound B2, reserved exactly as `merge_and` does.
        let mut out = Vec::with_capacity(a.len().min(b.len()) + SIMD_BLOCK);
        // SAFETY: the arm's feature was just asserted, and `out` is empty with
        // the capacity B2 requires.
        unsafe { simd::merge_and_into(a, b, &mut out) };
        out
    }
    ...
    fn vec_cardinality(a: &[u16], b: &[u16]) -> u32 {
        assert_vector_arm_reachable();
        // SAFETY: the arm's feature was just asserted.
