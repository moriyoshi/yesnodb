...
/// A handle to an event sink, cheap to clone.
///
/// The same shape as [`crate::dispatch::Dispatcher`], and for the same reason:
/// a collaborator belongs in `DbOptions` as a concrete type with a sensible
/// `Default`, not as an `Arc<dyn _>` in a public signature or as yet another
/// `open_with_*` constructor. Adding a third collaborator must not add a fourth
/// constructor, which is what `open_with_events` was on its way to doing.
#[derive(Clone)]
pub struct Events(Arc<dyn CoreEventSink>);

impl Events {
    /// Wrap a sink.
    pub fn new<S: CoreEventSink>(sink: S) -> Self {
        Self(Arc::new(sink))
    }

    /// Wrap a sink already behind an `Arc`, sharing it rather than re-boxing.
    ///
    /// The `Arc` is the caller's to keep: a test or a host that wants to
    /// inspect its own sink afterwards holds the concrete handle and hands a
    /// clone here.
    pub fn from_arc(sink: Arc<dyn CoreEventSink>) -> Self {
        Self(sink)
    }

    /// The default: events are published nowhere.
    pub fn noop() -> Self {
        Self(Arc::new(NoopEventSink))
    }
}

impl From<Arc<dyn CoreEventSink>> for Events {
    fn from(sink: Arc<dyn CoreEventSink>) -> Self {
        Self(sink)
    }
}
...
pub(crate) fn noop_sink() -> Events {
    Events::noop()
}

/// Publish without allowing an observer panic to affect database correctness.
pub(crate) fn emit(sink: &Events, event: CoreEvent) {
