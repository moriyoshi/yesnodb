...
/// measured. Construction and figures:
/// `.agents/docs/LTM/expression-planning-statistics-and-segmentation.md`, under
/// the source gate -- recorded there rather than cited by path, because the
/// crate that produced them lived in the scratch directory and is gone.
///
...
/// still rests on the synthetic table above.
///
/// That caution is not generic. It was the **third** figure the same consumer
/// gave for its own operand size, and the earlier two were wrong because the
/// unit moved between them rather than because anyone miscounted -- one chunk
/// inside a per-block cursor is not one expression operand. A consumer-reported
/// size is a measurement taken at *their* boundary, so it needs the boundary
/// stated before it can be compared with anything here. See
/// `.agents/docs/LTM/measurement-and-investigation-methodology.md`, under frames
/// and baselines.
...
fn cheap_to_reopen(e: &Expr) -> bool {
    ...
    // `.agents/docs/LTM/expression-planning-statistics-and-segmentation.md`,
    // under the source gate.
    matches!(
        e,
