# Install the version-locked Elasticsearch plugin into a disposable engine and
# compare its document matches with Python's set oracle.

db = db_open("search-elasticsearch")
db_insert_many(db, 42, [1, 3, 5])
db_insert_many(db, 91, [3, 4])
flight = flight_serve(db)
location = flight_location(flight)

assert search_engine_start(location, "elasticsearch") == "elasticsearch"
