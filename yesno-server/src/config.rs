...
impl From<DbConfig> for DbOptions {
    fn from(c: DbConfig) -> Self {
        DbOptions {
            ...
            // Collaborators are not configuration: a sink and an executor come
            // from the process that owns them, not from a TOML file. The
            // server sets `events` in `lifecycle`; `dispatch` stays sequential
            // until something supplies a runtime.
            ..Default::default()
        }
    }
}
