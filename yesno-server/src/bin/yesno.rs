...
impl QueryParser<'_> {
    ...
    fn reduce(&mut self) -> Result<yesno_flight::ViewReduce, QueryParseError> {
        let ident = self.ident()?.to_owned();
        if ident.eq_ignore_ascii_case("any") {
            Ok(yesno_flight::ViewReduce::Any)
        } else if ident.eq_ignore_ascii_case("all") {
            Ok(yesno_flight::ViewReduce::All)
        } else if ident.eq_ignore_ascii_case("parity") {
            Ok(yesno_flight::ViewReduce::Parity)
        } else {
            Err(self.error(format!(
                "unknown view reduction '{ident}'; expected any, all, or parity"
            )))
        }
    }
    ...
    fn expr(&mut self) -> Result<yesno_flight::SetExpr, QueryParseError> {
        ...
        if ident.eq_ignore_ascii_case("view-select") {
            self.expect(b'(')?;
            let key = self.number()?;
            self.expect(b',')?;
            let view = self.view()?;
            self.expect(b',')?;
            let at = self.at;
            let set = u32::try_from(self.number()?).map_err(|_| QueryParseError {
                at,
                message: "view constituent does not fit in u32".into(),
            })?;
            self.expect(b')')?;
            if set >= view.sets {
                return Err(self.error("view constituent is out of range"));
            }
            return Ok(yesno_flight::SetExpr::ViewSelect { key, view, set });
        }
        if ident.eq_ignore_ascii_case("view-fold") {
            self.expect(b'(')?;
            let key = self.number()?;
            self.expect(b',')?;
            let view = self.view()?;
            ...
            self.expect(b')')?;
            return Ok(yesno_flight::SetExpr::ViewFold { key, view, reduce });
        }
        if ident.eq_ignore_ascii_case("view-expand") {
            self.expect(b'(')?;
            let input = Box::new(self.expr()?);
            self.expect(b',')?;
            let view = self.view()?;
            self.expect(b')')?;
            return Ok(yesno_flight::SetExpr::ViewExpand { input, view });
        }
...
mod query_parser_tests {
    ...
    fn parses_every_documented_operator() {
        ...
        assert_eq!(
            parse_expression("view-select(9, interleaved(3), 2)").unwrap(),
            SetExpr::ViewSelect {
                key: 9,
                view: yesno_flight::ViewSpec::interleaved(3),
                set: 2,
            }
        );
        assert_eq!(
            parse_expression("view-fold(10, blocked(4, 65_536), parity)").unwrap(),
            SetExpr::ViewFold {
                key: 10,
                view: yesno_flight::ViewSpec::blocked(4, 65_536),
                reduce: yesno_flight::ViewReduce::Parity,
            }
        );
        assert_eq!(
            parse_expression("view-expand(and(1, 2), interleaved(3))").unwrap(),
            SetExpr::ViewExpand {
                input: Box::new(SetExpr::And(vec![SetExpr::Key(1), SetExpr::Key(2)])),
                view: yesno_flight::ViewSpec::interleaved(3),
            }
        );
    ...
    fn errors_name_the_location_and_problem() {
        for bad in [
            ...
            "view-select(9, interleaved(0), 0)",
            "view-select(9, interleaved(2), 2)",
            "view-fold(9, blocked(2, 0), maybe)",
        ] {
