...
def main() -> int:
    ...
    for key in new:
        # Print the **baseline key**, not only the site. `rel` is relative to
        # `docs/`, so a hit in `docs/data-modeling.md` has a key beginning
        # `data-modeling.md::` while the site beside it begins `docs/`. Anyone
        # reading this output to baseline an entry -- which the message below
        # invites -- would build the key from the site, get the prefix wrong,
        # and see it reported as **stale**: the opposite direction from the real
        # problem. Exercised on 2026-09-16 and it failed exactly that way. The
        # mechanism had never been run, because the baseline has always been
        # empty, and a safety net nobody has fallen into is a claim, not a net.
        print(f"NEW: {where[key]}: `{key.split('::', 1)[1]}`")
        print(f"     baseline key, if it must be deferred: {key}")
    for key in stale:
