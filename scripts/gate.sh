...
#
# Scoped to `broken_intra_doc_links` deliberately. The full `cargo doc` warning
# set is 38 here, almost all `private_intra_doc_links` ( a public doc linking to
# a private item ), which is a real but softer defect and each instance is a
# judgment: publish the item, or drop the link. Denying only this lint is a line
# that is at zero and can be held; denying all of them would need 38 decisions
# first and would be a baseline, not a gate.
check env RUSTDOCFLAGS="-D rustdoc::broken_intra_doc_links" \
