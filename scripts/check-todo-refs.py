...
# Bare stems of everything under `scripts/`, so prose naming a script is not
# mistaken for a backlog slug. Derived rather than listed: a hand-written list
# is the thing that goes stale, and this file exists because of that failure.
SCRIPT_STEMS: set[str] = {
    p.stem for p in (ROOT / "scripts").iterdir() if p.suffix in {".py", ".sh"}
} if (ROOT / "scripts").is_dir() else set()
            ...
            for lineno, line in enumerate(text.splitlines(), 1):
                for slug in SLUG.findall(line):
                    ...
                    if "/" in slug or slug.endswith((".rs", ".py", ".sh", ".md")):
                        continue
                    # Neither is a script's bare stem. A path like
                    # scripts/check-x-y.sh is excluded by the extension above,
                    # but prose naming the same script by its stem alone is
                    # lexically a slug and was flagged as a dangling citation.
                    # ( Both examples here are deliberately unbackticked, for
                    # the reason the module docstring gives: this file is inside
                    # the globs it scans, and the backticked form tripped this
                    # very check on the commit that added the exclusion. )
                    #
                    # The exclusion itself is NOT applied here. Dropping stems
                    # at the point of collection made the summary's "N script
                    # stem(s) not treated as slugs" structurally always zero --
                    # a sentence asserting an exclusion it could never observe,
                    # which is worse than not reporting one. They are collected
                    # like any other token and separated in `main`.
...
def main() -> int:
    ...
    sites = cited()
    # Counted, not merely skipped. Silently excluding a class of token leaves
    # the summary claiming more coverage than it has -- the same defect as a
    # line count that does not say comments were stripped. See "name what was
    # excluded" in QUALITY_GATE.md section 4.
    stems = sorted(s for s in sites if s in SCRIPT_STEMS)
    unresolved = {
        s: w
        for s, w in sites.items()
        if s not in corpus and s not in NOT_SLUGS and s not in SCRIPT_STEMS
    }
    ...
    if not new and not fixed:
        print(
            ...
            f"{len(NOT_SLUGS)} known non-slugs, {len(stems)} script stem(s) "
            f"not treated as slugs )"
        )
        for s in stems:
            print(f"    excluded as a script name: `{s}` ( {sites[s]} )")
