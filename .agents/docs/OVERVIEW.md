...
- **Not a bitmap DBaaS, an OLAP engine, a vector database, or a general authorization graph.** Those four exclusions are deliberate and are the boundary the product direction is drawn against, not an accident of what is unfinished. Ranking and content retrieval belong **outside** yesno.

### The intended product shape: an operational context-eligibility plane

The direction this is pointed at is a narrow service answering *which candidates are eligible*, over **rebuildable derived membership data** -- not a general-purpose store of record. Its first primitive is a filter from candidates, policies and minimum versions to a mask plus a **receipt**, the receipt naming the consumer snapshot, every share and share version, the authorization grant, and each source's ingestion watermark.

Two structural consequences follow and are worth stating here rather than only in the backlog:

- **A receipt needs a version vector, not a version.** Independently updated tenants have no naturally atomic global version, so a single number would be an invention rather than a summary.
- **Cross-tenant algebra needs an identity argument before it needs an implementation.** Two parties must be able to prove an ordinal names the same resource; natural shared identifiers or platform-issued ones qualify, and raw hashes of enumerable personal identifiers explicitly do not.

The initial commercial envelope is **BYOC** -- vendor-supported, in the customer's cloud, over data the customer can rebuild. Multitenant DBaaS is deferred until dedicated deployments have produced measured workload limits, p99 latency, RPO/RTO, tenant-isolation evidence and a working release pipeline. Rebuildability is what makes that envelope defensible: it bounds what a failure can cost.

These are **product decisions and documentation before they are deployment machinery**, and the backlog entries under "Product direction" own the detail.
