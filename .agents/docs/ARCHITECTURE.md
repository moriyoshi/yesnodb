...
`Tree::build_updating` keeps leaves no key touched, so a small edit rewrites a few nodes rather than the whole tree. Three rules make that work and all are load-bearing:

* **A leaf owns keys up to and including its own last key.** A key landing in the gap between two leaves therefore goes with the *following* one, and a key past the final leaf starts a new leaf rather than being absorbed into it — absorbing would rewrite a leaf that did not change.
* **A rebuilt range is split to end exactly on the previous leaf boundary**, or an inserted entry overflows the leaf and misaligns every boundary after it.
* **A leaf is reused only if no key in it was superseded and no changed key falls in its range** — a moved `ChunkRef` must not reuse a leaf naming the old extent.

**Untouched leaves are reused without being decoded** ( 2026-09-16 ). A leaf's first key is in its header and `key_at` is `O( 1 )`, so `leaf_spans` learns a leaf's key range from two indexed reads instead of parsing all its entries; only leaves a key actually falls in are decoded. This is what stops the checkpoint's exclusive region costing `O( total entries )` — measured at 70-82% of the hold before the change. The gain depends on **write layout** rather than write volume: contiguous writes touch few leaves and the hold falls severalfold, writes strided across the key space touch nearly all of them and it barely moves.

`Tree::build_reusing` is the older whole-slice builder. It has **no production caller** and is retained as the differential oracle for `build_updating`, on the same reasoning as the `roaring` dev-dependency: the two must agree on tree, height, lookups, carried count and superseded refs. They may differ in *which* pages are recycled, and only ever in the direction of `build_updating` recycling more — a positional slice comparison desynchronizes after a deletion, a key-range test does not.
...
### Commit completion precedes visibility, and read-your-writes is explicit

**A commit can be assigned, durable, and not yet readable.** `VersionOracle::advance` moves the watermark over a **consecutive prefix**, so a commit that resolves while an earlier one is still pending holds a version nothing can read. The window is bounded by one fsync.

That is a deliberate property, not a gap: the prefix rule is what makes a partially durable batch **invisible rather than half-visible**, and it is what allows the shard lock to be released before the fsync, which is what makes group commit effective.

**So read-your-writes is a sequence a caller performs, never an ambient guarantee**, and it has three parts: take the **acknowledged version** a write returns; `Db::wait_visible( version, timeout )` to wait the fsync window out; then read at **exactly** that version. Before `wait_visible` existed the API had no way to wait: `commit` returned a version whose own `snapshot_at` was refused, and that refusal was **indistinguishable from naming a version that will never exist** — the caller could not tell "too early" from "wrong".

The version request stays **exact** rather than becoming "at least this new". A coordinator fanning one query across endpoints depends on being given the version it asked for; a minimum-version form is a different request and needs its own wire variant rather than an overload of this one.
...
**The store is read before the memtable lock, not under it** ( 2026-09-16 ). Applying a batch needs the chunk each key already holds, and that used to be fetched lazily inside `mem.write()` through a miss-fallback closure -- a store-mutex acquisition, a B+tree descent and a checksum-verified container read, **once per row**, with every reader of that shard blocked behind it. A whole-key delete was worse: it needs the prefixes already on disk so each gets a tombstone, which is a range scan of the index. Both are now resolved in one pass **before** any memtable lock is taken, and the apply reads from that map.

Two things make it correct rather than merely faster. **I2** says a published extent is immutable, so a value read early cannot change underneath -- only become superseded, and the memtable is consulted first and is authoritative for anything newer. A concurrent *commit* cannot interfere at all, because every participant's `write` guard is already held. The one hazard is a concurrent **checkpoint**, which can persist a memtable chunk and evict it in between, leaving the prefetched copy older than the store; that is caught by comparing the shard's superblock sequence, read once per shard rather than per row, and a mismatch discards the whole prefetch and falls back to reading under the lock. The prefetch is also capped, because it holds every container at once where the apply loop read and dropped them one at a time.
