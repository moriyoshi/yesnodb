...
- A consumer's bit-sliced lens proposal was narrowed after measurement: `ops::csa` and `Slice::add_many` are not justified, `ge` / `top_k` require an independent product case, and only eager `OrdSet::view_count` remains requested.
...
A bit-sliced value would read several ordinary sets as integer planes over each ordinal, the transpose of `bignum`'s significance-inner layout. The storage doctrine fits because the caller still owns the layout and each plane remains an ordinary equality-encoded key. The arithmetic does not license a lazy API shape: `matrix/`, `bignum/`, `view/`, and `pack/` are eager and contain no `Expr` or `ChunkStream` integration.
...
`OrdSet::view_count` survives for a structural reason rather than a ratio: `view_fold` already computes the per-ordinal count and `Reduce::keep` discards it to a Boolean. Exposing that existing count adds no storage format, new lens, or lazy expression integration. The surviving request is limited to this eager count; the other parts have no shared justification.
