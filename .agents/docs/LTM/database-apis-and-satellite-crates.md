...
**MySQL's transaction boundary is honest by declaration, and tested as such.** `ha_yesno` advertises `HA_NO_TRANSACTIONS`, so MySQL warns that a non-transactional table changed and never expects rollback to undo anything; `external_lock` is a no-op and the handlerton registers no commit or rollback hooks, which is consistent with the declaration rather than missing from it. The fixture pins the behaviour instead of leaving it implied -- it runs `START TRANSACTION; INSERT ... ( 13 ); ROLLBACK` and then asserts the row is **still present**. That is the contrast with `yesno-pg`, which buffers per transaction to make `ROLLBACK` real and consequently owns a subtransaction problem MySQL cannot have: see [PostgreSQL Extension and Query Pushdown](./postgresql-extension-and-query-pushdown.md). **A declared limitation with a test is a smaller liability than an undeclared guarantee.**

**MySQL keeps the sequential default, and the reason is the handler contract rather than performance.** `Backend::Insert` is one key and one ordinal because `ha_yesno::write_row` takes its **duplicate-key verdict for a row from the return value of writing that row**: `changed == false` is the whole answer, and MySQL asks whether row N collided before it offers row N+1. A batch defers its commit, so `changed` for row N is not known until rows N+1.. have already been accepted -- by which time the engine has returned `0` for a row it must have rejected with error 1062, which the pinned MySQL 8.4 fixture checks by name. Batching the engine's writes would trade away per-row duplicate detection, a **declared behaviour of the table**, for parallelism that a single ordinal does not have anyway: one ordinal is one shard is one task.
