...
- `ops::array`, `ops::bitmap`, `ops::mixed` and `ops::run` each carry an x86_64 arm as well, and the arch test gap is closed at 910 tests on both.
- A kernel speedup is an **upper bound** on what the operation around it can gain; the isolated AVX2 popcount win of 2.70x becomes 1.17x at the container path.
...
**All four kernel modules now carry an x86_64 arm beside the NEON one** -- `ops::array` on SSE4.2 `pcmpestrm`, `ops::bitmap` on AVX2, `ops::mixed` and `ops::run` on SSE4.1 -- with 910 tests passing on both architectures. The array arm measures up to **8.51x** over scalar on real silicon at `m = 4096`, and the surviving container-path figures are 1.62x for `bitmap x bitmap`, 1.82x for `run x run` and 1.17-1.27x for the mixed pairs. Porting a kernel is not renaming intrinsics: x86 has no unsigned 16-bit compare, no lane de-interleave, a signed-only `madd_epi16` against a bound that permits 65535, and a reversed ANDNOT operand order. And the value of a *technique* belongs to the instruction set rather than to the algorithm -- Harley-Seal, the canonical AVX2 popcount win, loses by 2.9x on NEON, which has a cheap vector popcount. The arm-by-arm reasoning, the emulation rule, the rejected alternatives and the measurement discipline are in [SIMD Arch Arms and Kernel Selection](./simd-arch-arms-and-kernel-selection.md).
...
- Do not quote an emulated timing. `qemu` inverts a SIMD verdict rather than blurring it: 1.62x native, 0.87x emulated, no overlap.
- Do not benchmark a kind pair without first tracing which kernel answers it. `and_cardinality( array, run )` is answered by a scalar two-pointer in `ops::card`; `simd::filter_by_run` serves *apply* only, through `merge_array_run`.
- Apply the tree's sabotage discipline to performance claims too: disable the kernel, confirm the benchmark moves, and confirm it moves for **that kernel alone**. A figure that responds to an unrelated module's arm is a build artifact.
