...
| [GPU Offload on Unified Memory](./gpu-offload-on-unified-memory.md) | Why the PCIe argument does not apply on GB10, the one-memory bandwidth ceiling that caps single operations at 1.65x, the launch-granularity constraint, and the batched-reuse regime that is worth 9-11x. |
