...
### Transaction boundaries, and the subtransaction gap

A PostgreSQL transaction is not a yesno transaction: yesno commits per `WriteBatch`, PostgreSQL commits when it writes its commit record, and nothing lets one roll back the other. The wrapper's answer is to **buffer writes per transaction** in a thread-local and flush once in an `XACT_EVENT_PRE_COMMIT` callback. That buys a correct top-level `ROLLBACK` ( the buffer is discarded and nothing was ever sent ) and one server commit per transaction rather than one per row, which matters because a yesno commit appends to the WAL and fsyncs.

The buffer is a **map keyed by ordinal, last write wins**, not an insert list plus a remove list. Two lists cannot represent both orders: applying removals first makes `DELETE` then `INSERT` end present but leaves `INSERT` then `DELETE` present too, and applying insertions first just moves the error onto the other case. By the time `flush` runs the ordering information is gone, so keying by ordinal is what makes the last write survive -- which is what SQL means.

**Subtransactions needed their own callback, and the absence of one was a wrong-answer bug until 2026-09-19** ( `fdw-savepoints-are-not-honoured` ). `fdw/modify.rs` registered `RegisterXactCallback` only, and PostgreSQL delivers subtransaction aborts exclusively through `RegisterSubXactCallback` -- `XACT_EVENT_ABORT` fires at top level and nowhere else -- so nothing discarded the writes a rolled-back savepoint made. Measured against a hermetic cluster before the fix:

| construction | PostgreSQL semantics | wrapper, before |
|---|---|---|
| `INSERT 1; SAVEPOINT s; INSERT 2; ROLLBACK TO s` | `1` | `1, 2` |
| the same, read *inside* the transaction | `1` | `1, 2` |
| `RELEASE SAVEPOINT` | keeps the write | keeps the write ( correct ) |
| `INSERT 10, 20; SAVEPOINT s; DELETE 10; ROLLBACK TO s` | `10, 20` | **`20`** |
| nested savepoints, rollback to the outer | `10, 20, 30` | `20, 30, 40, 50` |
| PL/pgSQL `EXCEPTION` block ( implicit subtransaction ) | discards its writes | keeps them |

**The rolled-back `DELETE` row is the one to lead with**: it was not only "rows you rolled back get written" but **data loss** -- a `DELETE` inside a savepoint, rolled back, was still applied, so a row the user had restored disappeared. And the `EXCEPTION` case means it fired with no `SAVEPOINT` anywhere in the text.

**The fix is a stack of per-level maps**, keyed by `GetCurrentTransactionNestLevel()`, with a subtransaction callback beside the transaction one. `ROLLBACK TO` drops every level at or below the ending one, leaving the parent's entries untouched; `RELEASE` merges into the parent with the deeper verdict winning; reads and the flush both flatten ascending, so a transaction can neither see nor commit what it rolled back.

Three properties are load-bearing and each has a test that fails without it:

- **`RELEASE` must keep its writes.** A version discarding on every subtransaction end passes every rollback case and loses committed work.
- **The handlers are "at or below the ending depth", not "exactly it".** `ROLLBACK TO` an outer savepoint aborts every level from the innermost down to *and including* the named one, and PostgreSQL fires the event once per level; a handler matching one depth silently mishandles the arbitrary-savepoint case. The level whose rollback re-establishes it must then be usable again, since `ROLLBACK TO` starts a fresh subtransaction under the same name.
- **Per-entry subtransaction tags cannot work.** The per-ordinal map is last-write-wins, so an ordinal inserted by the parent and deleted by the child has already lost the parent's verdict when the rollback arrives. One map per level is what preserves it.

**The nesting level rather than the subtransaction id**, because these callbacks register lazily on the first *write*: a transaction whose first write happens inside a savepoint never saw `SUBXACT_EVENT_START_SUB`, so any design needing the start event has already lost its place. `PINNED` and `STATEMENT` are deliberately left alone -- a ticket pinned inside an aborted subtransaction over-retains a version until the transaction ends, which costs space and cannot serve wrong data.

**Distinct from `fdw-two-phase-commit`.** That is a crash window between the pre-commit flush and PostgreSQL's commit record, and closing it needs a durable prepare/resolve protocol yesno does not expose; a prepared `WriteBatch` cannot merely reserve a commit version, because the unresolved slot is a hole in the consecutive-prefix visibility oracle. The savepoint gap needs no yesno-side protocol at all, only correct bookkeeping in the wrapper, and it fires with no crash involved.

**Isolation-level pinning is correct and is a different axis.** REPEATABLE READ and SERIALIZABLE pin one ticket for the transaction; READ COMMITTED pins one per executor statement and refreshes between them, because holding a transaction-long ticket would hide other sessions' commits from a statement entitled to see them. The pin is released on **commit as well as abort** -- forgetting the commit half does not show up as a leak, it silently serves the next transaction in that backend stale data.
