...
- `Db::commit` holds the memtable write lock across the whole batch loop **by construction** -- atomicity, not oversight -- so its length is `O( records )` and only the record count or the structure can move it.
- The chunks and prefixes a batch needs are known from its planned units before any lock is taken. Resolving them ahead of `mem.write()` is sound under I2, and the residual hazard is a concurrent **checkpoint** evicting a memtable chunk, caught by the shard superblock sequence.
- Beyond `PREFETCH_MAX` the prefetch degrades to the old in-lock read, so its failure mode is the previous behaviour rather than a memory spike.
- There is exactly one lock pair in the tree: `mem.write()` then `store.lock()`, inside `disk_chunk`. The checkpoint path releases the store lock before taking `mem.write()`.
- Shard-invariance of a total stall does **not** rule out a lock: a query reading every shard against a checkpoint holding one shard at a time waits the total hold however it is divided.
- The per-shard work inside a commit is already independent -- every participant's `write` guard is held simultaneously in ascending order, which is what gives I5, and version assignment happens once above the loop -- so parallel apply does not weaken I5. Whether it pays is arithmetic, not structure.
...
### The memtable write lock, and what may be resolved outside it

`Db::commit` takes `shard.mem.write()` and holds it across the **entire batch loop**, because re-taking it per record would let a reader observe half a commit. The hold is therefore `O( records in batch )` **by construction**, and "hold the lock for less time" is not an available fix: the lever is the structure ( a shadow overlay, or a versioned memtable readers never need excluding from ) or the record count, not the section.

What that `O( records )` *contained*, however, was disk I/O rather than memtable mutation, and that part was removable without touching atomicity. Each unit's memtable miss fell back to `Shard::disk_chunk`, which takes the store mutex, descends the B+tree and reads a verified container -- inside the memtable write lock, and **once per row even for a key that does not exist**, so a fresh insert into a brand-new key paid a tree descent to learn the chunk was absent. The delete path had the same defect through `ShardStore::key_prefixes`, an index range scan run once per deleted key. Both are now resolved into a prefetch map **before any memtable lock is taken**, measured at 71-80% of the hold before and zero calls under the lock after.

**Why reading early is sound, and the one hazard it leaves.** Under **I2** a published extent is immutable, so a value read early cannot change underneath -- only become superseded -- and the memtable is consulted first and is authoritative for anything newer. The miss check stays under the lock because a concurrent commit may have filled it. The narrow real hazard is a concurrent **checkpoint** persisting a memtable chunk and evicting it inside the window, after which the memtable no longer holds it and the prefetched copy is *older* than the store; that is detected by comparing the shard's superblock sequence, read once per shard rather than per row. A concurrent commit cannot do this, because the path holds every participant's `write` guard.

**Lock ordering is consistent and there is exactly one pair anywhere**: `mem.write()` then `store.lock()`, inside `disk_chunk`. The checkpoint path drops the store lock before taking `mem.write()`. The combination reads like a deadlock risk and is not one; it was a latency problem.

The hold timings, the checkpoint-side exclusive region, and the tail-shape knobs that come out of all of this are in [Checkpoint and Commit Lock Contention](./checkpoint-and-commit-lock-contention.md). The one result to carry here: with **zero** checkpoints a writer still moves reader p99 from 4.38 ms to 82.61 ms, so commits own the latency tail and checkpoints own the worst case.
