...
pub fn expr_admits(expr: &SetExpr, key: u64, ordinal: u64) -> bool {
    match expr {
        ...
        //
        // `Hole` is listed for completeness rather than because it can arrive:
        // decoding refuses one outside a map body, and the FDW never builds a
        // map. Answering `false` is the conservative direction either way --
        // it can only omit a pending row, never invent one.
        SetExpr::At(..)
        | SetExpr::Fold(..)
        | SetExpr::Pack(..)
        | SetExpr::Expand(..)
